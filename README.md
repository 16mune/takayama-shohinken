...
> **【提供終了のお知らせ】**
> 本サイトは、高山市「暮らし応援商品券」の利用期間終了にともない、
> 加盟店検索の提供を **2026-09-01 に終了** しました。
> 現在 index.html は終了案内ページに差し替えており、加盟店データ（data/stores.json）と
> 自動更新ワークフロー（.github/workflows/update.yml）は削除済みです。
> 過去のデータ・検索ページは Git の履歴に残っています。
> データ取得スクリプト（scripts/update_data.py）は、次年度に同種の事業があった際の
> 再利用のため残してあります（自動では動作しません）。
